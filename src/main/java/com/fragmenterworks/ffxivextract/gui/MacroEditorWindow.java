...
class MacroEditorWindow extends JFrame {
    ...
    // FILE IO
    private final File lastOpenedFile = null;
    ...
    private final JLabel txtLineCounter;

    private final JLabel lblMacroChooserLabel;
    private final JLabel lblIconChooserLabel;
    private final JLabel lblMacroNameLabel;
    private final JLabel lblMacroBodyLabel;

    private final JButton btnSave;
    private final JButton btnSetMacro;
    ...
    private void doSave() {
        ...
        for (int i = 0; i < Macro_File.MAX_MACROS; i++) {
            //Title
            total += 4;
            total += currentMacro_File.entries[i].title.getBytes(StandardCharsets.UTF_8).length;
            ...
            for (int l = 0; l < Macro_File.MAX_LINES; l++) {
                total += 4;
                total += currentMacro_File.entries[i].lines[l].getBytes(StandardCharsets.UTF_8).length;
            }
        }
        ...
        for (int i = 0; i < Macro_File.MAX_MACROS; i++) {
            //Title
            bodyOut.put((byte) 'T');
            bodyOut.putShort((short) (currentMacro_File.entries[i].title.getBytes(StandardCharsets.UTF_8).length + 1));
            bodyOut.put(currentMacro_File.entries[i].title.getBytes(StandardCharsets.UTF_8));
            bodyOut.put((byte) 0);
            ...
            for (int l = 0; l < Macro_File.MAX_LINES; l++) {
                bodyOut.put((byte) 'L');
                bodyOut.putShort((short) (currentMacro_File.entries[i].lines[l].getBytes(StandardCharsets.UTF_8).length + 1));
                bodyOut.put(currentMacro_File.entries[i].lines[l].getBytes(StandardCharsets.UTF_8));
                bodyOut.put((byte) 0);
            }
        }
    ...
    class DocumentSizeFilter extends DocumentFilter {
        ...
        public void replace(FilterBypass fb, int offset, int length,
            ...
            if ((area.getText(rowBegin, rowEnd - rowBegin) + text + "\0").getBytes(StandardCharsets.UTF_8).length <= maxLength && !text.equals("\n")) {
                super.replace(fb, offset, length, text, attrs);
            }
