...
public class GraphicsNode {
    ...
    public GraphicsNode(ByteBuffer data) {
        ...
        type = data.getInt();
        if (type < 1001 && type != 1 && type != 2 && type != 3 && type != 4 && type != 5 && type != 8) {
            //TODO:GraphicsNodeTypeData_5の解析とクラス追加
            Utils.getGlobalLogger().trace(String.format("未知のGraphicsNode type「%s」があります。(アドレス:0x%08X)", type, (data.position() - 4)));
        }
